...
const notaDoPrimeiroBimestre = document.getElementById(
  "notaDoPrimeiroBimestre"
);
const notaDoSegundoBimestre = document.getElementById("notaDoSegundoBimestre");
const notaDoTerceiroBimestre = document.getElementById(
...
const mostrarResultado = () => {
  ...
  document.getElementById("notaFinal").innerHTML = `Nota final: ${res.toFixed(
    1
  )}`;
};
...
button.addEventListener(
  "click",
  (e) => (e.preventDefault(), mostrarResultado())
);

const getCelsius = document.getElementById("celsius");
const calculateTemperatureBtn = document.getElementById(
  "calculateTemperatureBtn"
);

const calculateTemperature = () => {
  const celsius = getCelsius.value;
  const fahrenheit = celsius * (9 / 5) + 32;
  document.getElementById("fahrenheit").innerHTML = `${fahrenheit.toFixed(
    1
  )} °F`;
};
